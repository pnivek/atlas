...
// Turbo8 dequant: load FP8 E4M3 data bytes + FP8 group scale → float.
// HDIM=256: data_ptr has 8 bytes (8 FP8 values), out[0..7]
// HDIM=128: data_ptr has 4 bytes (4 FP8 values), out[0..3]
// Dequant: val = fp8_to_f32(data) * fp8_to_f32(group_scale)
__device__ __forceinline__ void nvfp4_dequant(
    const unsigned char* data_ptr,
    const unsigned char* scale_ptr,
    const __half* lut,  // unused for turbo8 (kept for interface compat)
    float* out
) {
    // 2026-04-28: Turbo8 scales upgraded from FP8 to BF16 for precision.
...
#elif VEC_BF16 == 16
    // HDIM=512: 16 FP8 bytes per thread (2 × uint64 loads)
    unsigned long long pk_lo = *(const unsigned long long*)data_ptr;
    unsigned long long pk_hi = *(const unsigned long long*)(data_ptr + 8);
    out[0]  = fp8e4m3_to_f32((__nv_fp8_storage_t)(pk_lo & 0xFF)) * gs;
    out[1]  = fp8e4m3_to_f32((__nv_fp8_storage_t)((pk_lo >> 8) & 0xFF)) * gs;
    out[2]  = fp8e4m3_to_f32((__nv_fp8_storage_t)((pk_lo >> 16) & 0xFF)) * gs;
    out[3]  = fp8e4m3_to_f32((__nv_fp8_storage_t)((pk_lo >> 24) & 0xFF)) * gs;
    out[4]  = fp8e4m3_to_f32((__nv_fp8_storage_t)((pk_lo >> 32) & 0xFF)) * gs;
    out[5]  = fp8e4m3_to_f32((__nv_fp8_storage_t)((pk_lo >> 40) & 0xFF)) * gs;
    out[6]  = fp8e4m3_to_f32((__nv_fp8_storage_t)((pk_lo >> 48) & 0xFF)) * gs;
    out[7]  = fp8e4m3_to_f32((__nv_fp8_storage_t)(pk_lo >> 56)) * gs;
    out[8]  = fp8e4m3_to_f32((__nv_fp8_storage_t)(pk_hi & 0xFF)) * gs;
    out[9]  = fp8e4m3_to_f32((__nv_fp8_storage_t)((pk_hi >> 8) & 0xFF)) * gs;
    out[10] = fp8e4m3_to_f32((__nv_fp8_storage_t)((pk_hi >> 16) & 0xFF)) * gs;
    out[11] = fp8e4m3_to_f32((__nv_fp8_storage_t)((pk_hi >> 24) & 0xFF)) * gs;
    out[12] = fp8e4m3_to_f32((__nv_fp8_storage_t)((pk_hi >> 32) & 0xFF)) * gs;
    out[13] = fp8e4m3_to_f32((__nv_fp8_storage_t)((pk_hi >> 40) & 0xFF)) * gs;
    out[14] = fp8e4m3_to_f32((__nv_fp8_storage_t)((pk_hi >> 48) & 0xFF)) * gs;
    out[15] = fp8e4m3_to_f32((__nv_fp8_storage_t)(pk_hi >> 56)) * gs;
#elif VEC_BF16 == 4
...
#else
    #error "Unsupported VEC_BF16 (need 4, 8, or 16)"
#endif
}
...
extern "C" __global__ void paged_decode_attn_turbo8(
    ...
    __shared__ __half e2m1_lut[16];  // Reuse name to minimize code changes
    if (tid < 16) {
    ...
    const unsigned int head_scale_bytes = (head_dim / NVFP4_GROUP_SIZE) * 2;
